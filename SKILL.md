...
**Human gates.** A step with the `human-approval` check (the TDD flow's DISCOVERY) leaves only after a person approves it on the board: when verify reports it waiting, tell the user and point them at `agenfk ui --open <id>`. A person can also pass one blocked check with a written reason (an override) from the card on the board. Both are the human's alone: there is no CLI or MCP command for them, and the server refuses a request that carries the agent's token. Never try to approve or override on the user's behalf. Overrides and approvals are listed on the PR by `agenfk pr create`.
