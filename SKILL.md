...
6.  **Final Verification (Validate Tool)**
    ...
    *   **Test Suite Enforcement**: On the **final step** (and any boundary step) the server always runs the project's `verifyCommand` (set via `agenfk update-project <id> --verify-command "<cmd>"`); a command you pass there is ignored with a warning, and changing `verifyCommand` is recorded on every card in flight. On intermediate steps the command is optional and omitting it advances on the evidence alone. No advance is possible without evidence, which agents cannot fabricate or skip.
