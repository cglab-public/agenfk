...
async function run() {
    ...
    // 5. (no-op) The service script is SHIPPED, not generated.
    //
    // This step used to write a template over `rootDir/scripts/start-services.mjs`
    // - and from a source checkout rootDir IS the repo, so an install rewrote a
    // TRACKED file. The template had drifted behind the real script (it predated
    // the one-origin / UI-on-API change), so installing silently reverted local
    // work, and the suite's own "does not dirty tracked repo files" contract went
    // red. `agenfk up` reads the shipped script, so there is nothing to create.
