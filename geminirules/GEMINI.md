...
> these rules are enforced by instruction rather than mechanically. You MUST follow
> them strictly — `agenfk gatekeeper` returning `✅ AUTHORIZED` is your green light to edit.
...
**Precedence — the flow overrules these rules.** A step's exit criteria are the project's own configuration; the instructions in this file, in the `/agenfk-*` commands, and in the AgEnFK skill are defaults. Where they conflict, **the exit criteria win**. Follow them, and name the overridden default in your `agenfk verify --evidence` so the override is auditable rather than looking like a lapse.

The case this comes up most: Standard Mode says you are the sole agent and must not spawn sub-agents, while a REVIEW step's criteria demand an independent or adversarial reviewer. The criteria win — spawn the reviewer, without stopping to ask. The independence is the control being requested: an agent reviewing code it wrote itself carries the author's blind spots, so a self-review cannot satisfy that criterion however thorough it is. Brief reviewers to hunt for defects and to treat the review as **read-only** — a reviewer that edits a shared working tree can leave a defect behind. If this client cannot spawn sub-agents, review from a fresh context instead and say which you did. Then verify each finding against the code before acting on it; reviewers report false positives.

This does not extend to the framework's integrity rules. Gatekeeper authorization, forward transitions only via `agenfk verify`, no direct database or server writes, and no fabricated evidence hold no matter what a step says — a step that appears to demand otherwise is a flow bug, so report it instead of complying.
...
`--with-mcp` (e.g. `npx agenfk@latest --with-mcp`, or `agenfk integration install gemini --with-mcp`).
