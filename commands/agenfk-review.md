...
- Run `agenfk gatekeeper --item-id <id>` to confirm the active step, and read that step's `exitCriteria` from `agenfk flow show --project <projectId> --json`. That is the project's own definition of an acceptable review, and it **overrules the review method described below** — how deep the review goes, and whether it must be independent. It does not override Step 4's gating: the carve-out beneath this step lists what no flow may relax.
...
- Run `agenfk comment <id> "Review complete: audit and requirements traceability finished."` to log the step completion.
