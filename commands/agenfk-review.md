...
- If review passed: call `workflow_gatekeeper(id)` first (response includes exit criteria), then call `validate_progress(id, "<build_command>")` — pass a **compile/build command only**, never a test command. Advances to the next flow step on success (back to coding step on failure).
