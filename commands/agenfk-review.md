...
- If review passed: run `agenfk gatekeeper --item-id <id>` first (response includes the step's exit criteria), then `agenfk verify <id> --evidence "<summarize review findings and confirm criteria met>" ["<build_command>"]` — the command is optional; if you pass one, make it a **compile/build command**, never the test runner. Advances to the next flow step on success; a non-zero command refuses the advance and the item stays on this step.
