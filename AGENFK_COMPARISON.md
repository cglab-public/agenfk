...
| **Verification** | "Review all changes", "Run full test suite", PR review | Mandatory `verify_changes` tool, automated REVIEW &rarr; TEST transitions, 80% coverage enforcement via `enforce-coverage.ts` | **Covered** (AgenFK is more prescriptive) |
...
- **AgenFK**: Mechanically enforces the workflow. The `workflow_gatekeeper` rejects code changes unless a task is `IN_PROGRESS` and the agent's role matches the current phase. `verify_changes` prevents items reaching DONE without passing tests. A PreToolUse hook in CLAUDE.md provides an additional enforcement layer.
...
| **Coverage Enforcement** | Strict 80% coverage threshold enforced by `enforce-coverage.ts` and `verify_changes`, with per-file and overall checks. | Guidelines say "ensure tests" but define no numerical threshold. |
