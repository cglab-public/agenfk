  ...
  <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 p-4" role="dialog" aria-modal="true" aria-label={`Checks for ${step.label || step.name}`}
    ...
    <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white dark:bg-slate-900 shadow-xl">
      <div className="flex items-center gap-2 px-5 py-3 border-b border-slate-200 dark:border-slate-700">
        ...
        <button type="button" autoFocus aria-label="Close step checks" onClick={onClose} className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
          <X size={16} />
        </button>
      </div>
...
export const TemplatePicker: React.FC<{ onApply: (steps: FlowStep[]) => void }> = ({ onApply }) => {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-6 z-50 w-72 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg p-2 space-y-1">
