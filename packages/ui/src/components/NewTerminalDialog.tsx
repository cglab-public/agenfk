 ...
 *
 * One question, asked once: which agent. Everything else about how the terminal
 * runs — whether it survives quitting, whether the agent's permission prompts
 * are disabled — is a preference and lives in Settings.
 *
 * It did not start that way. Both were switches here, each with an explanatory
 * paragraph, and the dialog had become a form to fill in before doing the thing
 * you actually asked for. A dialog in the path of a frequent action earns its
 * fields; these two were answered the same way every time.
 *
 * Two hazards still shape it, neither of them visual:
 *
...
export function NewTerminalDialog({
  ...
  const chooseAgent = (next: string): void => { setAgentId(next); };
