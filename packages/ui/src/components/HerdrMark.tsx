 ...
 *
 * PROVENANCE. The path below is copied byte-for-byte out of herdr's own
 * `assets/logo.svg` (github.com/ogulcancelik/herdr, AGPL-3.0-or-later),
 * transform included. Nothing about the silhouette is redrawn. It identifies
 * the multiplexer these rows are about, the way a product's own mark does.
 * Verified identical to the copy collie carries, from the same source.
 *
 * ONE DEVIATION, AND IT IS THE CONTEXT'S. The original is a two-tone tile - a
 * `#303438` ram on a `#d9dad8` field. This draws the ram alone, in
 * `currentColor`, with no tile: the row it sits in is a list of monochrome
 * lucide glyphs, and a filled two-tone square among them reads as a foreign
 * object rather than as an icon. It also sidesteps the contrast trap collie
 * documented - `#d9dad8` against a light background is about 1.2:1, next to
 * invisible - without inventing a colour, because it takes whatever the text
 * around it already uses.
 *
 * The 512 grid and the flip in the transform are the original's; the viewBox
 * keeps them so the path lands where it was drawn to land.
 */
import React from 'react';

export function HerdrMark({ className, ...rest }: React.SVGProps<SVGSVGElement>): React.ReactElement {
  return (
    <svg
      /*
       * Labelled by default, because in the tree it is the ONLY thing saying
       * this row came from herdr. Forwarded props come last so the one caller
       * that needs it decorative - the agent icon, which always has the word
       * "herdr" in text beside it - can turn the label off rather than making
