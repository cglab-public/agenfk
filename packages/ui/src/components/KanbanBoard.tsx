    ...
    <div className="h-full min-h-screen bg-slate-100 dark:bg-slate-950 flex flex-col font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <main className="flex-1 overflow-x-auto p-4 md:p-6">
        <div className="flex flex-col md:flex-row gap-6 h-full">
          ...
          <div className={clsx("flex flex-col transition-all duration-300 h-full", isArchiveCollapsed ? "w-12 shrink-0" : "w-80 shrink-0")}>
            {isArchiveCollapsed ? (
              <button onClick={() => setIsArchiveCollapsed(false)} className="h-full bg-slate-200/50 dark:bg-slate-900/50 rounded-xl flex flex-col items-center py-6 gap-4 hover:bg-slate-300 dark:hover:bg-slate-800 transition-colors group border border-dashed border-slate-300 dark:border-slate-800">
                <Archive size={16} className="text-slate-500 group-hover:text-indigo-600 shrink-0" />
                <span className="[writing-mode:vertical-lr] font-bold text-[10px] uppercase tracking-widest text-slate-500 shrink-0">Archived</span>
                <span className="mt-auto bg-white dark:bg-slate-800 text-slate-500 text-[10px] font-bold px-1.5 py-0.5 rounded-full border border-slate-100 dark:border-slate-700">{items?.filter((i: AgenFKItem) => i.status === Status.ARCHIVED).length || 0}</span>
              </button>
            ) : (
