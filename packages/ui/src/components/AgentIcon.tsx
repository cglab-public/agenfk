/**
 * The vendors' own marks (CGLAB-169).
 *
 ...
 * colour is the weakest channel and fails outright for the ~8% of men with a
...
interface Mark {
  /**
   * A brand hex, or `currentColor` for marks that are monochrome by design.
   *
   * OpenAI's is the latter: the mark is black on light and white on dark, not
   * a colour. Hardcoding white would be wrong the moment the app is in light
   * mode, so it inherits the surrounding text colour instead.
   */
...
const MARKS: Record<string, Mark> = {
  ...
  codex: {
    // Monochrome by design — inherits the text colour, so it is white on the
    // dark shell and black in light mode without a second definition.
    color: 'currentColor',
    node: (
      <>
  ...
  // Not an agent, so not a brand: a shell prompt, stroked rather than filled.
  shell: {
    color: '#7C8496',
    node: (
      <path
        d="M5 7l5 5-5 5M12.5 17h6.5"
        fill="none"
        stroke="#7C8496"
        strokeWidth="2"
        strokeLinecap="round"
...
export function AgentIcon({ agentId, size = 16 }: AgentIconProps): React.ReactElement {
  ...
  if (!mark) {
    ...
    // instead of collapsing its row's layout. Asserted against in the picker
    // tests, because a fallback that lines up correctly is otherwise invisible.
    return (
      <span
        aria-hidden="true"
