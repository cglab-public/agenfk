...
describe('hub auth + setup', () => {
  ...
  it('GET /auth/providers reflects requiresSetup when no users exist', async () => {
    const r = await supertest(__server).get('/auth/providers');
  ...
  it('POST /setup/initial-admin creates the first admin', async () => {
    ...
    const r = await supertest(__server).post('/setup/initial-admin').send({ token: tok, email: 'first@x', password: 'longenough1' });
    expect(r.status).toBe(201);
    const r2 = await supertest(__server).post('/setup/initial-admin').send({ token: tok, email: 'second@x', password: 'longenough2' });
    expect(r2.status).toBe(409);
  });

  it('rejects short passwords on /setup/initial-admin', async () => {
    const tok = (await ctx.db.get('SELECT token FROM bootstrap_tokens LIMIT 1') as { token: string } | undefined)?.token;
    const r = await supertest(__server).post('/setup/initial-admin').send({ token: tok, email: 'a@b', password: 'short' });
  ...
  it('login fails with wrong password', async () => {
    ...
    const r = await supertest(__server).post('/auth/login').send({ email: 'alice@example.com', password: 'wrongpassword' });
    expect(r.status).toBe(401);
  });

  it('login is case-insensitive on email', async () => {
    await createPasswordUser(ctx.db, 'org', 'Alice@Example.com', 'rightpassword', 'viewer');
    const r = await supertest(__server).post('/auth/login').send({ email: 'alice@example.COM', password: 'rightpassword' });
    expect(r.status).toBe(200);
  });

  it('logout clears the session cookie', async () => {
    await createPasswordUser(ctx.db, 'org', 'a@b.com', 'rightpassword', 'viewer');
    const login = await supertest(__server).post('/auth/login').send({ email: 'a@b.com', password: 'rightpassword' });
    const cookie = login.headers['set-cookie']?.[0];
    const out = await supertest(__server).post('/auth/logout').set('Cookie', cookie);
    expect(out.status).toBe(200);
    const me = await supertest(__server).get('/auth/me').set('Cookie', out.headers['set-cookie']?.[0] ?? '');
    expect(me.status).toBe(401);
  });

  it('GET /auth/me without cookie returns 401', async () => {
    const r = await supertest(__server).get('/auth/me');
    expect(r.status).toBe(401);
  });
});
